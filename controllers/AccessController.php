...
class AccessController extends Controller
{
    ...
    */
    ...
    public function actionCreate($id)
    {
        ...
        if(!$note || $note->creator_id !== app()->user->id ) {
            throw new ForbiddenHttpException('No access');
        }
        $model = new Access();
        $model->note_id = $id;
        $users = User::find()->select('username')->exceptUser(app()->user->id)->indexBy('id')->column();

        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            app()->session->setFlash('success', 'The access to the note has been created '.$note->id);
    ...
    */
    ...
    public function actionDeleteAll($id)
    {
        ...
        $note->unlinkAll(Note::RELATION_ACCESSED_USERS, true);
        app()->session->setFlash('success', 'The access to the note has been removed '.$note->id);
